...
Run all tests with detailed names:
...
Run one TC by keyword:

```powershell
python -m pytest tests/test_checkout.py -k tc_034 -q
```

Run one exact test function:

```powershell
python -m pytest tests/test_checkout.py::test_tc_034_complete_purchase -q
```
...
Browser launch is controlled in `conftest.py`.

- `headless=True` runs the browser in the background
- `headless=False` opens the browser so you can watch the automation

If you want to see the browser while the test is running, change `headless=True` to `headless=False`.
